...
using multiple GPUs with samplers such as [MultiNest](https://github.com/JohannesBuchner/MultiNest).
...
To make use of multiple GPUs in an MPI run we will need to _bind_ the
...
OpenMPI we can use a modified [version](https://medium.com/@jeffrey_91423/binding-to-the-right-gpu-in-mpi-cuda-programs-263ac753d232) of `gpu_bind.sh`:
...
!!! tip
For the best performance, it is recommended to enable [MPS](https://docs.nvidia.com/deploy/mps/index.html) (Multi-Process Service) for the GPUs. This will allow multiple processes to prevent context switching. This will interleave transfers and compute between processes and significantly improve performance.
